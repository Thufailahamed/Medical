        ...
        <QueryBoundary query={query} loadingCount={3} emptyTitle="Test not found" emptyDescription="This diagnostic test is no longer available.">
          {(test: Detail) => {
            ...
            <div className="flex flex-col gap-4">
              ...
              <p className="text-sm font-semibold text-text">LKR {price} · {test.sampleType ?? "Sample collection"}{test.fastingRequired ? " · Fasting required" : ""}{typeof test.laboratoryCount === "number" ? ` · ${test.laboratoryCount} lab${test.laboratoryCount === 1 ? "" : "s"}` : ""}</p>
              {offers.length > 0 ? (
                <fieldset className="flex flex-col gap-2">
                  <legend className="text-xs font-semibold text-text-soft">Choose laboratory</legend>
                  {offers.map((o) => (
                    <label key={o.labId} className="flex items-center gap-2 text-sm text-text">
                      <input type="radio" name="lab" checked={(labId ?? effectiveLab) === o.labId} onChange={() => setLabId(o.labId)} />
                      <span className="font-medium">{o.labName || o.labId}</span>
                      <span className="text-text-soft">LKR {o.discountPrice ?? o.price}</span>
                    </label>
                  ))}
                </fieldset>
              ) : null}
              ...
              <button type="button" onClick={() => book(test, effectiveLab)} disabled={!date || !time || !addressLine1 || !city || !district || !contactPhone} className="self-start rounded-pill bg-brand px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">Book home collection</button>
            </div>
            );
          }}
        </QueryBoundary>
