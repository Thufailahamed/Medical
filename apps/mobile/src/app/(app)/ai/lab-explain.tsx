...
export default function LabExplainScreen() {
  ...
  async function explain(report: any) {
    ...
    // Use file URL if available, otherwise use report's stored text/summary.
    const fileUrl =
      report.pdfUrl ||
      report.fileUrl ||
      report.url ||
    ...
    // The server can use report.aiSummary as a hint via textHint.
    try {
      ...
      toast.show(err?.message || "Could not explain report", "danger");
    }
  }
  ...
  return (
    <Screen padded={false} edges={["top"]} bottomInset>
      <ScreenHeader
        ...
        onBack={() => router.back()}
        title="Lab explainer"
        subtitle="Plain-language explanation of your lab report"
      ...
      {result && selectedReport ? (
        ...
        <View style={{ padding: spacing.lg }}>
          <EmptyState
            ...
            message="Upload or fetch lab reports first."
            tone="neutral"
          />
        </View>
      ) : aiExplain.isPending ? (
        ...
        <View style={{ padding: spacing.lg, gap: spacing.md }}>
          ...
          {(reports?.reports || []).map((r: any) => (
            <ListItem
              ...
              pill={{ label: "Explain", tone: "primary" }}
              onPress={() => explain(r)}
            />
          ))}
        </View>
