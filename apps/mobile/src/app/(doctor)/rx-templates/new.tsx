...
export default function NewTemplateScreen() {
  ...
  const addMed = useCallback(() => {
    setDraft((d) => ({ ...d, medicines: [...d.medicines, { ...emptyMed }] }));
  }, []);

  const removeMed = useCallback((idx: number) => {
    setDraft((d) => {
      if (d.medicines.length <= 1) return d;
    ...
    <Screen padded={false} scroll edges={["top"]} style={{ backgroundColor: colors.bg }}>
      ...
      >
        ...
        <Field
          ...
          onChangeText={(text) => setDraft((d) => ({ ...d, diagnosis: text }))}
        ...
        <Field
          ...
          value={draft.notes}
          onChangeText={(text) => setDraft((d) => ({ ...d, notes: text }))}
